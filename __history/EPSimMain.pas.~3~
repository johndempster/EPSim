               ...
               if Cell.t >= (Cell.Stim.Start + Cell.Stim.Duration) then begin
                  ...
                  if rbSingleStimulus.Checked then begin
                     ...
                     if rbPairedStimulus.Checked and (Cell.Stim.NumStimDone >= 2) then begin
                        ...
                        outputdebugstring(pchar(format('stim start %.4g',[Cell.Stim.Start])));
                        Cell.Stim.I := Cell.Stim.Amplitude ;
                        end;
                     end ;
                  end ;
...
//    while nSteps > 0 do begin
       ...
       if Random < 0.0005 then GlutR := GlutR + 1.0 ;
       GlutR := Max(GlutR - (GlutR*Cell.dt)/GlutR_Tau,0.0); ;
       if rbEpilepsy.Checked then
          GlutR_Epilepsy := (1.0 - 0.01)*GlutR_Epilepsy + 0.01*(2.3+1.5*random)
          else
